...
const AlgorithmVisualizer = ({ algorithm = 'bubble', data = [64, 34, 25, 12, 22, 11, 90] }) => {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      for (let j = 0; j < n - i - 1; j++) {
        ...
        if (arr[j] > arr[j + 1]) {
          ...
          setArray([...arr]);
          await showAction(`交换 ${arr[j + 1]} 和 ${arr[j]}，因为 ${arr[j + 1]} > ${arr[j]}`);
        } else {
  ...
  const selectionSort = async () => {
    ...
    for (let i = 0; i < n - 1; i++) {
      ...
      if (minIdx !== i) {
        await showAction(`将最小值 ${arr[minIdx]} 与位置 ${i} 的 ${arr[i]} 交换`);
  ...
  const partition = async (arr, low, high, depth) => {
    ...
    for (let j = low; j < high; j++) {
      ...
      if (arr[j] < pivot) {
        i++;
        if (i !== j) {
          await showAction(`${indent}${arr[j]} < ${pivot}，交换 ${arr[i]} 和 ${arr[j]}`);
  ...
  const heapSort = async () => {
    ...
    for (let i = n - 1; i > 0; i--) {
      ...
      // 将当前最大元素（堆顶）移到数组末尾
      await showAction(`将堆顶最大元素 ${arr[0]} 与位置 ${i} 的 ${arr[i]} 交换`);
      setComparing([0, i]);
  ...
  const heapify = async (arr, n, i, phase) => {
    ...
    if (left < n) {
      setComparing([i, left]);
      await showAction(`比较节点 ${i}（${arr[i]}）和左子节点 ${left}（${arr[left]}）`);
    ...
    if (right < n) {
      ...
      await showAction(`比较当前最大值 ${arr[largest]} 和右子节点 ${right}（${arr[right]}）`);
    ...
    // 如果最大值不是根节点，则交换并继续调整
    if (largest !== i) {
      await showAction(`交换节点 ${i}（${arr[i]}）和节点 ${largest}（${arr[largest]}）`);
  ...
  const mergeSortHelper = async (arr, left, right, depth) => {
    if (left < right) {
      ...
      await showAction(`${indent}分割数组 [${left}, ${right}] 为 [${left}, ${mid}] 和 [${mid + 1}, ${right}]`);
      ...
      // 合并两个已排序的部分
      await showAction(`${indent}合并 [${left}, ${mid}] 和 [${mid + 1}, ${right}]`);
  ...
  const merge = async (arr, left, mid, right, depth) => {
    ...
    await showAction(`${indent}左数组: [${leftArr.join(', ')}], 右数组: [${rightArr.join(', ')}]`);
    ...
    while (i < leftArr.length && j < rightArr.length) {
      ...
      if (leftArr[i] <= rightArr[j]) {
        arr[k] = leftArr[i];
        await showAction(`${indent}${leftArr[i]} ≤ ${rightArr[j]}，将 ${leftArr[i]} 放入位置 ${k}`);
        i++;
      } else {
        arr[k] = rightArr[j];
        await showAction(`${indent}${leftArr[i]} > ${rightArr[j]}，将 ${rightArr[j]} 放入位置 ${k}`);
        j++;
      }
    ...
    while (j < rightArr.length) {
      arr[k] = rightArr[j];
      await showAction(`${indent}复制右数组剩余元素 ${rightArr[j]} 到位置 ${k}`);
      setArray([...arr]);
      j++;
      k++;
      await sleep(speed / 2);
    ...
    setComparing([]);
    await showAction(`${indent}合并完成: [${arr.slice(left, right + 1).join(', ')}]`);
  };
  ...
  const getBarColor = (index) => {
    ...
    if (heapRange.includes(index) && !sorted.includes(index)) return 'bg-orange-400'; // 堆范围
    if (mergeRange.includes(index) && !sorted.includes(index)) return 'bg-yellow-400'; // 归并范围
  ...
  const getAlgorithmDescription = () => {
    const descriptions = {
      bubble: '冒泡排序通过重复遍历数组，比较相邻元素并交换位置，让大的元素像气泡一样"冒"到数组末尾。时间复杂度：O(n²)',
      selection: '选择排序每次从未排序部分选择最小元素，放到已排序部分的末尾。时间复杂度：O(n²)',
      insertion: '插入排序将每个元素插入到已排序部分的正确位置，类似于整理扑克牌。时间复杂度：O(n²)',
      quick: '快速排序选择一个基准元素，将数组分为小于和大于基准的两部分，然后递归排序。平均时间复杂度：O(n log n)',
      heap: '堆排序首先构建最大堆，然后重复取出堆顶最大元素放到数组末尾，并重新调整堆。时间复杂度：O(n log n)',
      merge: '归并排序采用分治策略，将数组分成两半分别排序，然后合并两个已排序的数组。时间复杂度：O(n log n)'
    };
